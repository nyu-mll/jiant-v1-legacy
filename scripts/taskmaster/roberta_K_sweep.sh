...
function run_exp() {
    ...
    RANDOM_SEED=${3:-1234}
    # Add random seed
    OVERRIDES+=", random_seed=${RANDOM_SEED}, batch_size=2"
    OVERRIDES+=", cuda=\"auto\", accumulation_steps=2"
    echo ${OVERRIDES}
...
function hyperparameter_mlm_K_sweep_interm_train() {
    ...
    do
        OVERRIDES=" run_name=\"$1_mlm_new_$2_$K\", exp_name=roberta-large-K-new-mlm, pretrain_tasks=\"$1,wikipedia_corpus_mlm\", target_tasks=\"\",reload_tasks=0, reload_indexing=0, reload_vocab=0, weighting_method=\"examples_proportional_mixingK=$K\", input_module=roberta-large, do_target_task_training=0, transfer_paradigm=finetune, early_stopping_method=$1, do_pretrain=1, load_model=1"
...
#hyperparameter_mlm_K_sweep_interm_train qqp 111001  
