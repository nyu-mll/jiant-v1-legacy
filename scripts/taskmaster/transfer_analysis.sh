...
function hyperparameter_mlm_sweep_roberta() {
    ...
    for i in 0 1 3 6 7 #0 1 2 3 4 5 6 7
    do
...
function hyperparameter_mlm_sweep_albert() {
    ...
    for i in 0 1 2 3 4 5 6 7 
    do
        OVERRIDES="exp_name=my-experiment-large-albert-qqp-sop, pretrain_tasks=\"$1,wikipedia_corpus_sop\", target_tasks=\"\",reload_tasks=1, reload_indexing=0, reload_vocab=0, run_name=$1_sop_finetune_albert$i, input_module=albert-xxlarge-v2,batch_size=$2, do_target_task_training=0, transfer_paradigm=finetune, d_proj=4096, early_stopping_method=$1"
        EXP_OVERRIDES="${OVERRIDES}, run_name=$1_sop_finetune_k10k_albert$i"
        TASK_TYPE="regular"
        if [[ ${TASK_TYPE} == "edge" ]]; then
            BASE_CONFIG_FILE="base_edgeprobe"
        elif [[ ${TASK_TYPE} == "regular" ]]; then
...
hyperparameter_mlm_sweep_roberta qqp 6 111001
#hyperparameter_mlm_sweep_albert qqp 6 111001
