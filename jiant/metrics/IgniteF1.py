...
class _BasePrecisionRecall():
    ...
    def _check_shape(self, output):
        ...
        if self._is_multilabel and not (y.shape == y_pred.shape and y.ndimension() > 1 and y.shape[1] != 1):
            print('y.ndimension: {}'.format(y.ndimension()))
...
#                 import pdb; pdb.set_trace()
    ...
    def compute(self):
        ...
        if self._true_positives is None:
            self._true_positives = torch.DoubleTensor(0) if (self._is_multilabel and not self._average) else 0
#             print('self._true_positives is none.')
    ...
    def update(self, output):
        ...
        if self._type == "multilabel":
            if not self._average:
                ...
                if self._true_positives is None:
                    self._true_positive = torch.sum(true_positives / (actual_positives + self.eps))
                else:
...
#         print('PRED_SHAPE:{}'.format(preds.shape))
