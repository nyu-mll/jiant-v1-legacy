...
3. Create a class in ``src/tasks.py``, and make sure that...
    ...
    * Your task implements a method ``task.get_ssentences()`` that iterates over all text to index in order to build the vocabulary. For some types of tasks, e.g. ``SingleClassificationTask``, you only need set ``task.sentences`` to be a list of sentences (``List[List[str]]``).
